...
// HugerInteger implementation file for Assignment 3: HugeInteger.cpp
...
HugeInteger::HugeInteger( long long value )
{
    ...
    // UPDATE HERE
    if(value != 0LL){
        throw OverflowException("Operation caused overflow of HugeInteger");
    }
}
...
HugeInteger HugeInteger::operator +( const HugeInteger &op ) const
{
    HugeInteger temp = *this;
    temp += op;
    return temp;
...
HugeInteger & HugeInteger::operator +=( const HugeInteger &op )
{
    ...
    if(carry == 1){
        throw StringTooLargeException("String argument too large to be contained in a HugeInteger");
    }
...
void HugeInteger::input( const char *str )
{
    ...
    // convert individual digits of input value into a HugeInteger
    for( unsigned int j = NUMBER_OF_DIGITS - len; j < NUMBER_OF_DIGITS; ++j, ++k ){
        if (isdigit(str[k])){
            this->hugeInt[j] = str[k] - '0';
        } else {
            throw InvalidStringArgumentException("Invalid string argument - unable to convert into a HugeInteger");
        }
    }
}
...
ostream & operator << (ostream & dest, const HugeInteger & value)
{
    // find first non-zero digit
    unsigned int i = 0;
    while(i < HugeInteger::NUMBER_OF_DIGITS){
    ...
    if (i == HugeInteger::NUMBER_OF_DIGITS)
    {
        dest << '0';
        return dest;
    }
    ...
    for( ; i < HugeInteger::NUMBER_OF_DIGITS; ++i)
    {
